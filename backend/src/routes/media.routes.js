...
router.get("/summary", async (req, res, next) => {
  try {
    ...
    res.json(await getSummary(start, end, isFiltered, campanha, veiculo, modeloCompra));
  } catch (err) {
    next(err);
  }
});
...
router.get("/performance", async (req, res, next) => {
  try {
    ...
    res.json(await getPerformanceSeries(start, end, metrics, campanha, veiculo, modeloCompra));
  } catch (err) {
    next(err);
  }
});
